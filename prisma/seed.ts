...
const SeedCommand = async () => {
  // ADVANCE KYC NOT ENROLLED IN PROJECT
  // const usersWithoutHajipurOrOneRejectedHajipur = await prisma.user.findMany({
  //   where: {
  //     membership: 'ADVANCE',
  //     OR: [
  //       {
  ...
  // usersWithoutHajipurOrOneRejectedHajipur.map((data) => {
  //   console.log(data.pw_id, data.name, data.membership, data.mobile_number);
  // });

  const filePath = '/Users/apple/Downloads/5Rama.csv';
  const dataset = [];
